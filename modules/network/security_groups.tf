...
resource "aws_vpc_security_group_ingress_rule" "allow_inbound_ssh_connection_public" {
  ...
  ip_protocol       = "tcp"
  tags = {
    Name = "Allow ssh connections to public subnet"
  }
  depends_on = [aws_security_group.sg]
}

resource "aws_vpc_security_group_ingress_rule" "allow_inbound_icmp_public" {
  for_each          = local.public_subnets_map
  security_group_id = aws_security_group.sg[each.key].id
  from_port         = -1
  to_port           = -1
